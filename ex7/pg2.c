...
int in = 0, out = 0;
int words_processed = 0, total_words = 5;
...
const char *dictionary[DICT_SIZE] = {"Security", "Cyber", "Terminal", "Department"};
...
int main() {
    printf("\n1. Spell check program using Pthreads with conditional variables and mutex\n");
    const char *words_to_check[] = {"Department", "Values", "Terminal", "Blackout", "Cyber"};
    ...
    for (int i = 0; i < result_count; i++) {
        printf("Word to be checked: %-8s | Result: %s\n",
               result_buffer[i],
               result_status[i] ? "Valid" : "Invalid");
    }
    pthread_mutex_unlock(&mutex);
